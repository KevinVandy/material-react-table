...
- v0.40.1 - new `tableInstanceRef` and `virtualizerInstanceRef` props
- v0.39.0 - Renamed `enableColumnFilterChangeMode` to `enableColumnFilterModes`, renamed `enableGlobalFilterChangeMode` to `enableColumnFilterModes`. New `renderColumnFilterModeMenuItems` and `renderGlobalFilterModeMenuItems` props.
