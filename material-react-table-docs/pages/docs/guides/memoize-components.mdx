...
The reason these features are incompatible with cell memoization is because these features depend on table cells being re-rendered.
...
The reason these features are incompatible with row memoization is because they require the entire row or cells to be re-rendered in order to update the UI.
