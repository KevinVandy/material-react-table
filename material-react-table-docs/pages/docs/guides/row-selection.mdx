...
There are two callbacks that can be used to respond to selection changes. `onMrtSelectRowChange` is called when a row is selected or deselected. `onMrtSelectAllChange` is called when the select all checkbox is selected or deselected.
