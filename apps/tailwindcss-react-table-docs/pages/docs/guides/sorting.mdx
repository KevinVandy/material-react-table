...
<Head>
  <title>Sorting Feature Guide - TailwindCSS React Table Docs</title>
  <meta
    name="description"
    content="How to use, customize, or disable sorting features in TailwindCSS React Table"
  />
</Head>

## Sorting Feature Guide

TailwindCSS React Table supports almost any sorting scenario you may have. Client-side sorting is enabled by default, but you can opt to implement your own server-side sorting logic, or even replace the default client-side sorting with your own implementation.
...
By default, TailwindCSS React Table will use an `alphanumeric` sorting function for all columns.
...
> When `manualSorting` is set to `true`, TailwindCSS React Table assumes that your `data` is already sorted by the time you are passing it to the table.
...
Here is the full Remote Data example showing how to implement server-side sorting, filtering, and pagination with TailwindCSS React Table.

<RemoteExample />

View Extra Storybook **[Examples](https://www.tailwindcss-react-table.dev/?path=/story/features-sorting-examples)**
