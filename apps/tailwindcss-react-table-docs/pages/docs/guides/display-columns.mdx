...
<Head>
  <title>{'Display (Built-in) Columns - TailwindCSS React Table Docs'}</title>
  <meta
    name="description"
    content="How to create or edit display columns in TailwindCSS React Table"
  />
</Head>
...
TailwindCSS React Table has a few built-in display columns that are created automatically when certain features are enabled.
...
Display columns are, for the most part, the same as a data column, except they do not have an accessor to access data. When a display column is created internally by TailwindCSS React Table, the following options are all set to false by default:
