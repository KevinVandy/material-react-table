...
<Head>
  <title>Row Ordering/DnD Feature Guide - TailwindCSS React Table Docs</title>
  <meta
    name="description"
    content="How to use and customize the row drag and drop and row ordering features of TailwindCSS React Table"
  />
</Head>

## Row Ordering (DnD) Feature Guide

TailwindCSS React Table has exposes all the APIs necessary to enable rich row drag and drop features that you can easily build to meet your needs. This includes the ability to reorder rows, drag rows to other tables, or drag rows to other UI in your application.
