...
<Head>
  <title>
    {
      'Expanding Sub-Rows/Tree Data Feature Guide - TailwindCSS React Table Docs'
    }
  </title>
  <meta
    name="description"
    content="How to add expanding sub-rows or tree data features in TailwindCSS React Table"
  />
</Head>

## Expanding Sub-Rows (Tree Data) Feature Guide

TailwindCSS React Table has support for expanding sub-rows or tree data. This feature is useful for displaying hierarchical data. The sub-rows can be expanded and collapsed by clicking on the expand/collapse icon.
...
However, your data must also be formatted in a way to allow for expanding rows that are in some way related to each other. By default, TailwindCSS React Table will look for a special `subRows` property on each row of your data and treat any array of rows that it finds as the sub-rows for that row. You can customize or override this behavior by passing a custom `getSubRows` prop.
...
By default, TailwindCSS React Table will show the expand all button in the expand column header. You can disable this by setting the `enableExpandAll` prop to `false`.
...
By default, TailwindCSS React Table will treat expanded sub-rows the same as any other row when it comes to pagination. This means that some expanded rows may be on the next page. You can change this behavior by setting the `paginateExpandedRows` prop to `false`.
