...
<Head>
  <title>Pagination Feature Guide - TailwindCSS React Table Docs</title>
  <meta
    name="description"
    content="How to use, customize, or disable pagination features in TailwindCSS React Table"
  />
</Head>

## Pagination Feature Guide

Client-side pagination is enabled by default in TailwindCSS React Table. There are a number of ways to customize pagination, turn off pagination, or completely replace the built-in pagination with your own manual or server-side pagination logic.
...
```tsx
<TailwindCSSReactTable columns={columns} data={data} enablePagination={false} />
```
...
The default pagination features are client-side. This means you have to have all of your data fetched and stored in the table all at once. This may not be ideal for large datasets, but do not worry, TailwindCSS React Table supports server-side pagination.

When the `manualPagination` prop is set to `true`, TailwindCSS React Table will assume that the `data` that is passed to the table already has had the pagination logic applied. Usually, you would do this in your back-end logic.
...
View Extra Storybook **[Examples](https://www.tailwindcss-react-table.dev/?path=/story/features-pagination-examples)**
