...
- Fix issue nested grouped columns and column re-ordering sizes
...
- Fix issue with column resizing css vars when `"+"` are in accessor keys

#### v1.7.2 (2023-02-09)

- Fix issue with column resizing css vars when spaces are in accessor keys

#### v1.7.1 (2023-02-08)

- Fix issue with column resizing css vars when using nested accessor keys with dot notation
...
- Fix row dragging borders not showing in semantic layout mode
...
- Fix alert banner z-index issue
...
- Fix issue with memoized rows not sorting correctly with row virtualization enabled
- Fix issue with exiting full screen causing virtualized rows to disappear
...
- Fix global filtering with manual pagination when on page 2 or greater
