...
If you are using TypeScript, you may also want to import some TypeScript types from `'material-react-table'` to take advantage of the advanced strong type checking and autocompletion features. The `TRT_ColumnDef` type is especially useful for defining your columns in a type-safe way.

```tsx
import MaterialReactTable from 'material-react-table';
import type { TRT_ColumnDef } from 'material-react-table'; // If using TypeScript (optional, but recommended)
```
