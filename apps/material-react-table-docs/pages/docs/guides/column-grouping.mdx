...
## Manual Grouping with Nextjs SSR
        ...
        GroupedCell: ({ row, table }) => {
          ...
          return (
            <>
              ...
              {row.original?.count > 0 ? `(${row.original?.count})` : ''}
            </>
          );
        },
      },
...
```
You can get search params on your page.tsx 
```jsx
//page.tsx
...
let catchGroup: string[] = Object.entries(searchParams).filter((item: any) => item[0].startsWith('group'));
const groups: string[] = catchGroup.map((item: any) => item[1]);
//your backend logic may be diffrent
const groupData = await fetchGroup(GROUP_QUERY, {
groupBy: groups
})
.
.
