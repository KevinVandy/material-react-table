...
<Head>
  <title>Row Pinning Feature Guide - Material React Table V2 Docs</title>
  <meta
    name="description"
...
You can enable the row pinning feature with the `enableRowPinning` prop. There are multiple display modes that you can choose from with the `rowPinningDisplayMode` prop.
