...
<Head>
  <title>Pagination Feature Guide - Material React Table V2 Docs</title>
  <meta
    name="description"
...
You can customize the pagination component with the `muiPaginationProps` prop to change things like the `rowsPerPageOptions` or whether or not to show the first and last page buttons, and more.
